...
def draw(imgL, boxes, scores, classes, fps, imgR):
    ...
    for box, score, cl in zip(boxes, scores, classes):
        ...
        #print("空间坐标是：", int(threeD[y][x][0]), int(threeD[y][x][1]), int(threeD[y][x][2]), "mm")
...
while(cap.isOpened()):
    ...
    if boxes is not None:
        draw(img_1, boxes, scores, classes, fps, imgR)
 
    duration = dt.datetime.utcnow() - start
    print(round(duration.microseconds / 1000))
    # show output
